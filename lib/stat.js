 ...
 *
 * @private
 * @param  {Number} rows    Amount of rows to clear
 * @param  {Number} cols    Amount of columns to clear
 */
function clear(rows, cols) {
    process.stdout.moveCursor(0, -rows);
    for (var i = 0; i < rows; i++) {
        process.stdout.write(align('', cols * 2) + '\n');
    }
    process.stdout.moveCursor(0, -rows);
}
...
function display(err, stats) {
    ...
    var keys = ['count:n', 'avgObjSize:b', 'size:b', 'storageSize:b', 'indexSizes:b', 'totalIndexSize:b'];
    var opts = this;
    ...
    stats = keys.map(function buildLine(key) {
        ...
        if (key[0] === 'indexSizes') {
            ...
            subkeys.unshift(align(key[0] + ':', opts.size[0] + opts.size[1]));
        ...
        return pad + align(key[0], opts.size[0] - pad.length) +
            align(val.value, opts.size[1], {type: 'right', color: 'yellow'}) + ' ' + val.suffix;
    }.bind(stats));
    ...
    clear(rows, opts.size[0] + opts.size[1]);
